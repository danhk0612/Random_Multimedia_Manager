...
## 기본 완성 후 실사용 검증 — 이관, 미실시

2026-09-21 사용자는 대부분의 테스트를 기본 완성 후 실사용하면서 진행하도록 지시했다. T12는 구현과 Windows 자동 검증을 근거로 완료 처리하며 아래 잔여 수동 항목은 완료 차단 조건에서 분리한다. 이관은 검증 통과를 뜻하지 않는다. PR 병합은 별도이며 후속 Task는 이번 작업에서 진행하지 않는다.

아래 항목은 기본 완성 후 사용 가능한 버전에서 확인한다. 결과를 확인할 때 버전/커밋, 만화·영상 구분, 재현 순서, 기대/실제 동작과 오류 내용을 기록한다. 삭제·복구 확인에는 계속 테스트용 복사본을 사용한다.
...
DB/저널 장애는 자동 주입 검사로 검증하며 실사용 DB 권한을 변경해 재현하지 않는다. T12의 이관 근거는 이번 사용자 승인이다. 기존 Task의 자동·수동 검증 결과와 미검증 기록은 그대로 유지한다.
