...
이 문서는 구현 중 가장 쉽게 모호해질 수 있는 다음 규칙을 고정한다.
...
단순 전체 경로만으로 파일을 식별하면 이름 변경/이동 시 새 파일처럼 보일 수 있다.

초기 식별 우선순위:

1. 기존 `NormalizedPath` 정확히 일치
2. 일치하지 않으면 `FileSize + LastWriteTimeUtc + 선택적 Fingerprint` 조합으로 이동/이름 변경 후보 탐색
3. 확신할 수 없으면 새 파일로 등록

### Fingerprint

대용량 영상 전체 SHA 해시는 기본값으로 사용하지 않는다.

후속 구현 후보:

- 파일 앞/뒤 일부 블록 + 파일 크기 기반 빠른 fingerprint
- 충돌 가능성을 감안하여 자동 병합은 보수적으로 수행

잘못된 기록 승계보다 새 항목으로 인식하는 편이 낫다.

## 4. 랜덤 후보 계산

기본 전역 설정을 `HistoryExclusionDays`라고 한다.
...
- 즉시 종료 기능에서 현재 Pending 기록 보존 안 함 정책이 적용됨
...
- 즉시 종료 기능에서도 이미 시작된 DB transaction을 가능한 한 정상 종료한 뒤 앱을 닫음
