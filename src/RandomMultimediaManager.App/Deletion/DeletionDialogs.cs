    ...
    public static async Task RecoverAsync(DeletionService service, Window? owner = null, SessionCoordinator? coordinator = null)
    {
        foreach (var entry in service.Pending.ToArray())
        {
            if (entry.Record is { Phase: DeletionPhase.Succeeded or DeletionPhase.Failed or DeletionPhase.Cancelled } record)
            {
                ...
                {
                    var retry = await coordinator.ResolveDeletionAsync(Guid.NewGuid(), service, entry, record.Phase == DeletionPhase.Succeeded);
                    if (retry.Status != SessionStatus.Completed || retry.Error is not null)
                        Show(owner, retry.Error ?? "삭제 복구를 완료하지 못했습니다.");
                }
                else
                {
            ...
            string message = entry.Record is null
                ? $"삭제 저널이 손상되어 대상을 확인할 수 없습니다.\n{entry.File}\n{entry.Error}\n\n‘아니요’는 삭제 실패/취소로 확인하여 모든 기록을 보존합니다. 성공 처리는 대상을 알 수 없어 불가능합니다. 확인할 수 없으면 ‘취소’를 선택하세요. 확인 가능한 경로는 해당 경로를, 경로도 알 수 없으면 라이브러리 전체를 격리합니다."
