...
| T04 | 분류와 소스 폴더 UI | Sol | 완료 (사용자 Windows UI 확인, PR #9 통합 대기) | T03 |
...
- T04는 자동 검증과 2026-09-09 사용자 Windows UI 수동 검증을 모두 완료했다. PR #9가 main에 통합되면 저장소 기준 T04 선행이 완전히 해제된다.
- 바로 병렬 가능: T08 만화 표시·조작, T10 외부 SRT/SMI. 둘 다 Sol 범위이며 구조/공통 계약 변경은 Astra로 분류한다.
- T08의 D06 및 T10의 D07 기본값은 해당 담당자가 위임 범위에서 문서화하고 구현한다.
- T05는 T04 완료 결과 PR #9가 main에 통합된 뒤 시작한다. 그때 진행 중인 T08/T10과 병렬 가능하다. T06은 T05 이후다.
- 각자 별도 브랜치. T08은 만화, T10은 영상 자막, T05는 스캔만 담당한다. MainWindow의 진입/종료, App.csproj·솔루션·상태 문서는 공유 파일이므로 병합 시 각 Task의 최신 변경을 보존한다. 자동 병합 성공만으로 충분하다고 보지 말고 통합 빌드로 네임스페이스·참조·종료 수명을 확인한다.
- T09 승인 생략은 후속 Task 검증 생략 승인이 아니다.
