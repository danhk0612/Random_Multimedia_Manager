...
T01은 사용자 Windows 검증 확인으로 완료했고 T02 계약은 main에 통합되었다. T03은 PR #4에서 구현·Windows 자동 검증을 완료했고 main에 병합되었다. T04는 PR #5에서 구현 및 자동 검증을 완료했으나 실제 Windows UI 수동 조작 검증 전이므로 완료 처리하지 않는다. T07(Sol), T09(Astra)의 준비 상태는 그대로 보존한다. 정책과 위임 범위는 docs/DECISIONS.md를 따른다.
...
- 담당/상태: Astra / 실측 대기, 미완료 (자동 검증 통과, PR #7 Draft).
- 선행: T01 완료/T02 계약 통합/T03 공유 모델.
