...
| T10 | 외부 SRT/SMI 자막 | Sol | 검증 대기 (자동 검증 완료, PR #11) | T09; D07 자막 결정 |
...
- T10은 PR #11에서 D07 기본값 문서화·구현·Windows 자동 검증까지 완료했고, 실제 화면의 한글 자막 표시·영상 대비 싱크·UI 클릭 흐름 수동 확인을 기다린다. 자동 검증 결과를 수동 통과로 간주하지 않는다.
...
- 목적: 한글 외부 자막 표시.
- 담당/상태: Sol / 검증 대기 (PR #11, Windows 자동 검증 완료; 실제 표시/싱크 수동 미검증).
- 선행: T09; D07 자막 결정.
...
- 남은 검증: 자동 검증은 자막 텍스트/`<SYNC>` 보존과 LibVLC 트랙 연결을 확인했으나 실제 데스크톱 화면의 한글 표시와 영상 대비 시간 싱크, 실제 UI 클릭 흐름은 관찰하지 않았다. 미수행을 통과로 처리하지 않으므로 T10 완료 처리는 보류한다.
