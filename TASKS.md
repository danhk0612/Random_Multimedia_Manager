...
| T07 | ZIP/CBZ 페이지 읽기 | Sol | 완료 (PR #6 통합 대기) | T01/T02/T03 |
...
T01은 사용자 Windows 검증 확인으로 완료했고 T02 계약은 main에 통합되었다. T03은 PR #4에서 구현·Windows 자동 검증을 완료했고 main에 병합되었다. T04는 PR #5에서 구현 및 자동 검증을 완료했으나 실제 Windows UI 수동 조작 검증 전이므로 완료 처리하지 않는다. T07은 PR #6에서 구현·Windows 자동 검증을 완료했으며 통합 대기다. T09(Astra)의 준비 상태는 그대로 보존한다. T08은 T07이 main에 통합된 뒤 별도 작업으로 착수한다. 정책과 위임 범위는 docs/DECISIONS.md를 따른다.
