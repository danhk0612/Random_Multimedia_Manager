...
| T01 | Windows 빌드 및 셸 실행 확인 | Sol | 검증 대기 | T00 |
| T02 | 데이터와 감상 세션 계약 설계 | Astra | 계약 완료 | T00; D01/D02 확정·위임 정책 적용, T01과 설계 병렬 가능 |
| T03 | SQLite 저장 기반 | Astra | 대기 | T01 완료 및 T02 계약 확정 |
...
T01은 Windows 검증 대기이고 T02 문서 계약은 완료하여 PR #1/#2/#3과 함께 main에 통합했다. T01 재개 절차는 CURRENT_STATE.md를 따른다. 병렬 문서 설계는 허용되며 후속 코드는 아직 착수하지 않는다. D01/D02의 사용자 결정과 나머지 세부 정책 위임은 docs/DECISIONS.md에 반영되어 있다. T02는 코드/프로젝트 파일을 변경하지 않는다. T03 등 후속 코드 구현 착수 전에는 T01 완료와 T02 계약 확정을 모두 확인한다.
...
- 현재 검증: PR #1 미병합과 기준 커밋, README 명령, 솔루션/App의 WPF 시작 구성을 정적 확인했다. 현재 작업 환경에 Windows 실행 환경이 없어 restore·build·실행·닫기는 미검증이며 T01은 완료하지 않는다.
...
- 수정하지 말아야 할 영역: 스키마 의미 재결정·뷰어·트레이.
