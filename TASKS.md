...
| T08 | 만화 표시와 조작 | Sol | 검증 대기 (자동 검증 완료, PR #10) | T07; D06; T02 계약 |
...
- T08은 PR #10에서 구현과 Windows 자동 검증을 완료했고 실제 Windows UI/실파일 시각·입력 수동 검증만 남아 있다. D06 기본값은 docs/DECISIONS.md에 확정했다.
- 바로 병렬 가능: T10 외부 SRT/SMI, T04 잔여 Windows UI 검증, T08 잔여 Windows UI/실파일 검증. 모두 Sol 범위이며 구조/공통 계약 변경은 Astra로 분류한다.
- T10의 D07 기본값은 해당 담당자가 위임 범위에서 문서화하고 구현한다.
- T05는 T04 완료 결과가 main에 통합된 뒤 시작한다. 그때 진행 중인 T08/T10과 병렬 가능하다. T06은 T05 이후다.
...
- T09 승인 생략은 T04/T08이나 후속 Task 검증 생략 승인이 아니다.
...
- 목적: 자체 만화 감상 화면 완성.
- 담당/상태: Sol / 검증 대기 (PR #10 구현·Windows 자동 검증 완료, 실제 Windows UI/실파일 수동 검증 미수행).
- 선행: T07; D06; T02 계약.
...
## T08 잔여 검증 새 Sol Chat 지시문

```text
T08의 남은 Windows UI/실파일 수동 검증과 필요한 T08 범위 수정만 진행해.
저장소: https://github.com/danhk0612/Random_Multimedia_Manager
먼저 최신 main, PR #10, task/t08-comic-viewer의 병합 여부와 PROJECT.md, REQUIREMENTS.md, ARCHITECTURE.md, CURRENT_STATE.md, TASKS.md, AI_WORKFLOW.md, docs/DECISIONS.md, docs/VIEWER_PLAYER_SPEC.md 및 관련 만화 코드를 확인해.
PR #10이 아직 미병합이면 그 브랜치의 최신 변경을 기준으로 검증하고, 이미 병합됐으면 최신 main에서 별도 T08 검증 브랜치를 만들어. 병렬 T04/T10 변경을 덮어쓰지 마.

D06 기본값과 자동 검증 결과를 재결정하지 말고 실제 Windows 화면에서 실 ZIP/CBZ를 열어 한 페이지/두 페이지/세로 연속 스크롤, LTR/RTL, 페이지 경계, 원본/창/폭/높이 맞춤, Ctrl+휠 확대, 왼쪽 드래그 팬, 표시 품질을 확인해. 가능하면 일반 DPI와 다른 배율에서도 확인해.
파일 전환과 창 닫기 뒤 원본 ZIP/CBZ를 이름 변경/이동/삭제할 수 있는지 확인하고, 손상 이미지는 Ready/활성 전환 없이 실패 안내가 되는지 확인해. 자동 검사 34320005257의 시작 페이지 Ready·취소·진행 복원·256 MiB LRU·잠금 해제 결과를 보존해.
문제가 있으면 T08 범위만 최소 수정해. DB·랜덤·기록·영상·T10 자막·트레이·추가 압축 형식·AI 확대·캐시 설정 UI는 변경하지 마. MainWindow의 T04 화면과 T09 영상 종료 대기 경로를 보존해.
Windows 수동 검증 환경이 없으면 구체적인 사용자 검증 절차를 제공하고 T08 검증 대기를 유지해. T09의 검증 생략 승인을 T08에 적용하지 마.
실제 Windows 빌드·자동 검사·수동 확인을 구분해 CURRENT_STATE.md, TASKS.md 및 영향을 받은 기준 문서만 갱신하고 커밋·PR·검증 결과·남은 제약을 보고해. 직접 병합하거나 다음 Task로 진행하지 마.
```
