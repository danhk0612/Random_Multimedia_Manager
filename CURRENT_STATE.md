...
| 외부 SRT/SMI 자막 | T10 구현 및 Windows 자동 검증 완료, 실제 화면 표시·싱크 수동 확인 대기 (PR #11) |
...
T10은 task/t10-external-subtitles / PR #11에서 D07 기본값 문서화와 구현, Windows 자동 검증을 완료했다. 실제 데스크톱에서 자막 문구 표시와 영상 대비 싱크를 눈으로 확인하지 않았으므로 완료가 아니라 수동 검증 대기 상태다. 자막 실패는 영상 Ready/방문 상태를 변경하지 않으며 T09의 토큰·준비/활성·해제, 장치 부재 영상 전용, 끝 위치 완료 복원 정책을 유지한다.
...
## T10 외부 SRT/SMI — 자동 검증 완료, 수동 표시 검증 대기
...
- 자동 검증은 SMI의 `<SYNC>` 마크업과 한글 텍스트 보존 및 LibVLC 트랙 연결을 확인했지만, 실제 데스크톱 화면에서 한글 문구가 보이는지와 영상 대비 시간 싱크가 맞는지는 관찰하지 않았다. 이 두 항목과 실제 UI 클릭 흐름은 수동 검증 대기로 남기며 통과로 표기하지 않는다.
