...
T00 완료, T01 Windows 검증 대기, T02 계약 완료(PR 검토/통합 대상). 후속 코드 구현은 T01 완료 및 T02 계약 통합 전 시작하지 않는다.
...
통합 시 TASKS/CURRENT_STATE의 T01 최신 검증 결과를 우선 보존하면서 T02 병렬 허용과 확정 계약을 되돌리지 않는다. PR #1→#2→T02 순서로 기반을 통합하거나 동일 변경을 포함한 기준을 확인한다.
