...
| 만화 표시·조작 | T08 구현·Windows 자동 검증 및 사용자 Windows 수동 검증 완료 (PR #10 병합 대기) |
...
T08은 task/t08-comic-viewer / PR #10에서 구현·자동 검증·사용자 Windows 수동 검증을 완료했다. 수동 검증 중 창 초기화 NRE, 비 seek ZIP 엔트리 직접 디코딩에 따른 부분 표시, 리사이즈 렌더 지연, 비동기 프리로드의 동시 ZIP 읽기로 인한 후반 페이지 부분 디코딩을 발견해 T08 범위에서 수정했고 최종 사용자 확인에서 표시·모드 전환·맞춤·리사이즈·후반 페이지 진행에 추가 문제 없음으로 확인했다. 최신 헤드 `694aaa85d4f1a9503a7982f452c5174504036136`에서 T03/T07/T09 Actions 회귀도 모두 성공했다. PR #10은 병합 대기이며 직접 병합하지 않는다. D06 기본값은 docs/DECISIONS.md에 확정했다. T10과 T04 잔여 UI 검증은 병렬 가능하다. T05는 T04 완료·통합 후, T06은 T05 후, T11은 T06 완료와 T08 PR #10 main 통합 후 진행한다. 구체적인 지시문은 TASKS.md를 따른다. T14 생명주기 상세(특히 숨김 상태 종료 저장 실패), T10 자막, T18 배포 검증은 해당 Task에 남긴다. T02 데이터 정책 자체의 추가 사용자 결정은 없다.
