...
| 만화 표시·조작 | T08 구현·Windows 자동 검증 완료, 실제 Windows UI 수동 검증 중 창 초기화 NRE 발견·수정 후 재검증 대기 (PR #10) |
...
T08은 task/t08-comic-viewer / PR #10에서 구현과 Windows 자동 검증을 완료했고 실제 Windows 수동 검증을 시작했다. 2026-09-09 사용자 PC에서 만화 뷰어 버튼 클릭 시 종료되는 문제가 재현되었고, 진단 로그로 XAML 초기화 중 `FitModeChanged -> UpdateUi()`가 아직 생성되지 않은 `ZoomText`를 참조하는 `NullReferenceException`임을 확인했다. `UpdateUi()`가 `PageText`와 `ZoomText` 모두 초기화된 뒤에만 갱신하도록 수정했으며 재수동 검증 대기다. D06 기본값은 docs/DECISIONS.md에 확정했다. T10과 T04 잔여 UI 검증은 병렬 가능하다. T05는 T04 완료·통합 후, T06은 T05 후, T11은 T06/T08 완료 후 진행한다. 구체적인 지시문은 TASKS.md를 따른다. T14 생명주기 상세(특히 숨김 상태 종료 저장 실패), T10 자막, T18 배포 검증은 해당 Task에 남긴다. T02 데이터 정책 자체의 추가 사용자 결정은 없다.
...
- 2026-09-09 사용자 Windows 수동 검증에서 `만화 뷰어` 클릭 직후 앱이 종료되는 문제가 확인되었다. 진단 로그 결과 XAML 로딩 중 `FitModeBox`의 초기 선택 변경 이벤트가 `UpdateUi()`를 호출할 때 `PageText`는 이미 생성됐지만 뒤쪽의 `ZoomText`는 아직 생성되지 않아 `NullReferenceException`이 발생한 것이 원인이었다. `UpdateUi()`가 두 컨트롤 모두 생성된 경우에만 갱신하도록 수정했다. 이전에 원인 분리를 위해 추가한 초기 Skia 렌더 지연/창 시작 진단은 유지하며, 이 수정 이후 실제 UI 재검증이 필요하다.
...
- **미검증:** 위 초기화 결함 수정 이후 실제 사용자 Windows 화면에서 만화 뷰어 창이 정상 생성되는지, 실 ZIP/CBZ를 열어 이미지가 시각적으로 올바르게 표시되는지, 한/두/세로 모드 전환과 읽기 방향, Ctrl+휠 확대·맞춤·팬, DPI별 시각 품질/입력 감각을 직접 확인하지 않았다. 자동 테스트 통과를 이 수동 검증 통과로 간주하지 않는다.
