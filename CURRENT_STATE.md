...
### T09 사용자 실파일 검증 및 후속 수정 (2026-09-09)

- 사용자 로그: 음성 포함 MP4/MKV 각 3회 프로브 정상 완료, 복사본 잠금/이동/삭제 통과. UI 일반·소프트웨어 영상/음성 재생 사용자 확인. 실제 GPU 사용·누출·전체화면 등은 확인되지 않았다. 로컬 HEAD/SDK는 미제공.
- 사운드 장치 부재 오류와 프로브 창 응답 없음이 보고되었다. 별도 task/t09-readiness-fixes / PR #8에서 UI 스레드 파일 복사 제거, 해제 전 준비 실패 진단 보존, 비경계 탐색 실패의 숨김/음소거 재시작과 0/1/2초 위치 검사를 보완했다. Windows 코드 fe9cdf3의 영상(34298340730)·기존 회귀(34298340739) 검사는 통과했다. 끝 위치 별도 진단에서는 6초 MP4가 Ended/vout=0으로 Ready 실패했다.
- 장치 부재에서 오디오 준비 검사를 생략하지 않는다. 정확한 끝 위치 Ready와 장치 부재 처리, 데스크톱 미확인 항목 때문에 T09 미완료를 유지한다. 상세는 docs/VIDEO_ENGINE_VALIDATION.md.

### T09 승인 예외 구현

- 사용자가 장치 부재의 영상 전용 준비와 끝 위치의 완료 상태 복원을 승인했다. 위의 승인 전 계약 차단은 이 보완으로 대체한다.
- Windows 기본 장치의 E_NOTFOUND만 --no-audio로 처리하고 장치 연결 후 다시 열기 안내를 표시한다. 오디오 자동 복구는 하지 않는다.
- 끝 위치는 실제 시작 디코딩을 숨김/일시정지로 보유하고 완료 상태·끝 진행으로 활성화한다. 명시적 재생에서 같은 방문으로 처음부터 시작한다. 검증 창에 끝 복원 체크를 추가했다.
- 기본 프로브에 합성 음성 MP4와 끝 위치 동작 assertion을 추가했다. 코드 844d8a0의 Windows 영상 검사 34298998375에서 빌드 경고/오류 0, 실제 장치 부재 영상 전용과 세 파일의 끝 위치/재생/토큰/해제 검사가 통과했다. 수정 후 사용자 장치 부재/재연결·누출·전체화면·트랙·HW 검증은 남아 있으며 T09 미완료를 유지한다.
